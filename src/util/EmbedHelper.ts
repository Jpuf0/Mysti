...
export default class EmbedHelper {
  private json: EmbedOptions;
	constructor(defaults = true, user?: User | null, json: EmbedOptions = {}) {
		this.json = json ?? {};
		if (defaults) {
			this.setColor("bot").setTimestamp("now").setFooter("Mysti", botIcon);
			if (user) this.setAuthor(`${user.username}#${user.discriminator}`, user.avatarURL);
		}
	}

	static new(defaults = true, user?: User) { return new EmbedHelper(defaults, user); }
